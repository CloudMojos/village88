...
SELECT customer_id, first_name, last_name, email FROM customer WHERE MONTH(create_date) = 2 AND active = 1;
-- 12. Retrieve customer records (email field as well as email_length field) where the customer with the longest email address is shown first.  If customers have the same length of email address, order the list by the email field in the ASC order (meaning it shows the email address that starts with 'A' first).
